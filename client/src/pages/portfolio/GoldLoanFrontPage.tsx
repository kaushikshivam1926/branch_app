/*
 * GoldLoanFrontPage.tsx
 * ─────────────────────────────────────────────────────────────────────────────
 * Renders a pre-filled A4 front-page cover sheet for a Gold Loan (GOLD series)
 * file. Data is fetched from LOAN_DATA in IndexedDB.
 ...
 *   - Ornament Description (editable)
 *   - Total Weight in grams (editable)
 *   - Purity / Carats (editable)
 *   - Appraised Value (editable)
 *   - LTV % (auto-computed from LIMIT / appraised value)
 *   - Appraiser Name (editable)
 *   - Appraisal Date (editable)
 *   - Pledge Register Folio No. (editable)
 */
...
const CHECKLIST_ROWS: { left: string; right: string }[] = [
  { left: "Gold Loan Application Form", right: "Gold Appraisal Certificate" },
  { left: "KYC Documents", right: "Pledge Letter (Form-A)" },
  { left: "UID / Voter ID / Passport", right: "Gold Delivery Receipt" },
  { left: "Photograph (Passport Size)", right: "Valuation Report" },
  { left: "PAN Card (Self-attested)", right: "Insurance Certificate" },
  { left: "Address Proof", right: "Gold Loan Agreement" },
  { left: "IT Undertaking", right: "NeSL Disclosure Consent" },
  { left: "Borrower Acknowledgement", right: "Sanction Letter" },
  { left: "Register Entry", right: "Control Return" },
];
...
function buildPrintHTML(params: {
  ...
  const detailRows: [string, string][] = [
    ...
    ["Outstanding Balance", outstandingAmt ? formatCurrency(outstandingAmt) : ""],
    ["Rate of Interest", intRate ? `${intRate.toFixed(2)}% p.a.` : ""],
  ...
  const goldRows: [string, string][] = [
    ["Ornament Description", ornamentDesc],
    ["Total Weight (grams)", totalWeight],
    ["Purity / Carats", purity],
    ["Appraised Value (₹)", appraisedValue],
    ["LTV %", ltvPct],
    ["Appraiser Name", appraiserName],
    ["Appraisal Date", appraisalDate],
    ["Pledge Register Folio No.", pledgeRegFolio],
  ];

  const detailRowsHtml = detailRows.map(([label, value]) => `
    <tr style="border-bottom:0.5px solid #e0e0e0;">
      <td style="padding:1.8mm 3mm;font-weight:600;color:#333;width:46mm;vertical-align:top;white-space:nowrap;border-right:0.5px solid #e0e0e0;background:#f8f9fc;">${label}</td>
      <td style="padding:1.8mm 3mm;color:#111;vertical-align:top;white-space:pre-wrap;">${value || blank()}</td>
    </tr>`).join("");

  const goldRowsHtml = goldRows.map(([label, value]) => `
    <tr style="border-bottom:0.5px solid #e0e0e0;">
      <td style="padding:1.8mm 3mm;font-weight:600;color:#333;width:46mm;vertical-align:top;white-space:nowrap;border-right:0.5px solid #e0e0e0;background:#fffbf0;">${label}</td>
      <td style="padding:1.8mm 3mm;color:#111;vertical-align:top;">${value || blank()}</td>
    </tr>`).join("");
  ...
  const checklistHtml = CHECKLIST_ROWS.map((row, i) => `
    <tr style="border-bottom:${i < CHECKLIST_ROWS.length - 1 ? "0.5px solid #ddd" : "none"};">
      <td style="width:50%;padding:1.5mm 3mm;border-right:0.5px solid #ddd;">
        <span style="display:inline-block;width:3mm;height:3mm;border:1px solid #333;margin-right:2mm;vertical-align:middle;"></span>${row.left}
      </td>
...
<div class="page">
  ...
  <div style="border-bottom:3px solid #b8860b;margin-bottom:4mm;padding-bottom:3mm;">
    <div style="display:flex;align-items:center;gap:8mm;">
      ...
      <div style="flex:1;">
        ...
        <div style="font-size:8.5pt;color:#555;margin-top:0.5mm;">Gold Loan File — Pledge Register</div>
      </div>
      <div style="text-align:right;">
        <div style="font-size:17pt;font-weight:bold;color:#b8860b;border:2px solid #b8860b;padding:1.5mm 4mm;border-radius:4px;letter-spacing:1px;">${serialNo}</div>
        <div style="font-size:7pt;color:#888;margin-top:0.5mm;">Gold File No.</div>
      </div>
    </div>
  </div>

  <!-- Customer Details -->
  <table style="margin-bottom:3mm;font-size:8pt;border:0.5px solid #e0e0e0;">
    <tbody>${detailRowsHtml}</tbody>
  </table>

  <!-- Gold Ornament Details -->
  <div style="border:1.5px solid #b8860b;border-radius:3px;overflow:hidden;margin-bottom:3mm;">
    <div style="background:#b8860b;color:white;padding:1.5mm 4mm;font-size:8.5pt;font-weight:bold;letter-spacing:0.3px;">Gold Ornament Details</div>
    <table style="font-size:8pt;border:0.5px solid #e0e0e0;"><tbody>${goldRowsHtml}</tbody></table>
  </div>

  <!-- Documentation Checklist -->
  <div style="border:1.5px solid #003399;border-radius:3px;overflow:hidden;margin-bottom:3mm;">
    <div style="background:#003399;color:white;padding:1.5mm 4mm;font-size:8.5pt;font-weight:bold;letter-spacing:0.3px;">Documentation Checklist</div>
    <table style="font-size:7.5pt;"><tbody>${checklistHtml}</tbody></table>
  </div>
    ...
    <tbody>
      <tr>
        <td style="padding:2mm 3mm;border-right:1px solid #aaa;width:50%;">
          ...
          <span style="border-bottom:1px solid #333;display:inline-block;min-width:28mm;">&nbsp;</span>
        </td>
        <td style="padding:2mm 3mm;width:50%;">
          <strong>Document Execution Register</strong><br/>
          <span style="color:#555;">Folio No.: </span>
          <span style="border-bottom:1px solid #333;display:inline-block;min-width:14mm;">&nbsp;</span>
          &nbsp;/&nbsp;
          <span style="border-bottom:1px solid #333;display:inline-block;min-width:14mm;">&nbsp;</span>
        </td>
      </tr>
  ...
  <!-- Footer -->
  <div style="margin-top:3mm;font-size:7pt;color:#888;text-align:center;border-top:0.5px solid #ddd;padding-top:2mm;">
...
export default function GoldLoanFrontPage({ record, onClose }: Props) {
  ...
  const outstandingAmt = loanData?.OUTSTAND || 0;
  const intRate = loanData?.INTRATE || 0;
  const sanctionDate = loanData?.SANCTDT || record.sanctionDate || "";
  const maturityDate = loanData?.Maturity_Dt || "";
  ...
  // Auto-compute LTV when both sanctionAmt and appraisedValue are available
  const ltvPct = (() => {
    const av = parseFloat(appraisedValue.replace(/[^0-9.]/g, ""));
    if (av > 0 && sanctionAmt > 0) return `${((sanctionAmt / av) * 100).toFixed(1)}%`;
    return "";
  })();
  ...
  const handlePrint = () => {
    ...
    const html = buildPrintHTML({
      ...
      custName, cifNo, acctNo, acctDesc,
      sanctionAmt, outstandingAmt, intRate,
  ...
  const inputStyle: React.CSSProperties = {
    width: "100%", border: "none", borderBottom: "1.5px solid #b8860b",
    outline: "none", fontSize: "8.5pt", fontFamily: "Arial, Helvetica, sans-serif",
    color: "#111", background: "transparent", padding: "0.5mm 0",
  };

  const GoldRow = ({ label, value, onChange, placeholder }: {
    label: string; value: string; onChange: (v: string) => void; placeholder?: string;
  }) => (
    <tr style={{ borderBottom: "0.5px solid #e0e0e0" }}>
      <td style={{ padding: "1.8mm 3mm", fontWeight: 600, color: "#333", width: "46mm", verticalAlign: "top", whiteSpace: "nowrap", borderRight: "0.5px solid #e0e0e0", background: "#fffbf0" }}>
        {label}
      </td>
      <td style={{ padding: "1mm 3mm", verticalAlign: "top" }}>
        <input type="text" value={value} onChange={e => onChange(e.target.value)}
          placeholder={placeholder || `Enter ${label.toLowerCase()}…`} style={inputStyle} />
      </td>
    </tr>
  );
      ...
      <div className="fixed inset-0 z-50 bg-black/70 flex flex-col items-center justify-start overflow-auto py-6 px-4">
        ...
        <div className="w-full max-w-[220mm] flex items-center justify-between mb-4 flex-shrink-0">
          <div className="text-white font-semibold text-lg">
            Gold Loan Front Page — <span className="font-mono text-yellow-300">{record.serialNo}</span>
          </div>
          <div className="flex gap-2">
            <button onClick={handlePrint} disabled={loading || printing}
              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-yellow-600 text-white text-sm font-semibold hover:bg-yellow-700 transition-colors disabled:opacity-50">
        ...
        {loading ? (
          ...
          <div className="bg-white shadow-2xl rounded flex-shrink-0"
            style={{ width: "210mm", minHeight: "297mm", padding: "10mm 13mm", fontFamily: "Arial, Helvetica, sans-serif", fontSize: "9pt", color: "#000", boxSizing: "border-box" }}>

            {/* Header */}
            <div style={{ borderBottom: "3px solid #b8860b", marginBottom: "4mm", paddingBottom: "3mm" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8mm" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: "8.5pt", color: "#555", marginTop: "0.5mm" }}>Gold Loan File — Pledge Register</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: "17pt", fontWeight: "bold", color: "#b8860b", border: "2px solid #b8860b", padding: "1.5mm 4mm", borderRadius: "4px", letterSpacing: "1px" }}>
                    {record.serialNo}
                  </div>
                  <div style={{ fontSize: "7pt", color: "#888", marginTop: "0.5mm" }}>Gold File No.</div>
                </div>
              </div>
            </div>

            {/* Customer Details */}
            <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: "3mm", fontSize: "8pt", border: "0.5px solid #e0e0e0" }}>
              <tbody>
                {[
                  ["Name of Customer", custName],
                  ["CIF Number", cifNo],
                  ...
                  <tr key={i} style={{ borderBottom: "0.5px solid #e0e0e0" }}>
                    <td style={{ padding: "1.8mm 3mm", fontWeight: 600, color: "#333", width: "46mm", verticalAlign: "top", whiteSpace: "nowrap", borderRight: "0.5px solid #e0e0e0", background: "#f8f9fc" }}>{label}</td>
                    <td style={{ padding: "1.8mm 3mm", color: "#111", verticalAlign: "top" }}>
                      {value || <span style={{ borderBottom: "1px solid #999", display: "inline-block", minWidth: "60mm" }}>&nbsp;</span>}
                    </td>
                  </tr>
                ))}
              </tbody>
            ...
            {/* Gold Ornament Details */}
            <div style={{ border: "1.5px solid #b8860b", borderRadius: "3px", overflow: "hidden", marginBottom: "3mm" }}>
              <div style={{ background: "#b8860b", color: "white", padding: "1.5mm 4mm", fontSize: "8.5pt", fontWeight: "bold" }}>Gold Ornament Details</div>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "8pt", border: "0.5px solid #e0e0e0" }}>
                <tbody>
                  <GoldRow label="Ornament Description" value={ornamentDesc} onChange={setOrnamentDesc} placeholder="e.g. Necklace, Bangles, Earrings…" />
                  <GoldRow label="Total Weight (grams)" value={totalWeight} onChange={setTotalWeight} placeholder="e.g. 25.5 g" />
                  <GoldRow label="Purity / Carats" value={purity} onChange={setPurity} placeholder="e.g. 22 Carat / 916 Hallmark" />
                  <GoldRow label="Appraised Value (₹)" value={appraisedValue} onChange={setAppraisedValue} placeholder="e.g. 1,50,000" />
                  <tr style={{ borderBottom: "0.5px solid #e0e0e0" }}>
                    <td style={{ padding: "1.8mm 3mm", fontWeight: 600, color: "#333", width: "46mm", verticalAlign: "top", whiteSpace: "nowrap", borderRight: "0.5px solid #e0e0e0", background: "#fffbf0" }}>LTV %</td>
                    <td style={{ padding: "1.8mm 3mm", color: ltvPct ? "#b8860b" : "#111", fontWeight: ltvPct ? "bold" : "normal" }}>
                      {ltvPct || <span style={{ color: "#999", fontSize: "7.5pt" }}>Auto-computed from Sanction Amount ÷ Appraised Value</span>}
                    </td>
                  </tr>
                  <GoldRow label="Appraiser Name" value={appraiserName} onChange={setAppraiserName} />
                  <GoldRow label="Appraisal Date" value={appraisalDate} onChange={setAppraisalDate} placeholder="DD/MM/YYYY" />
                  <GoldRow label="Pledge Register Folio No." value={pledgeRegFolio} onChange={setPledgeRegFolio} />
                </tbody>
              </table>
            </div>

            {/* Documentation Checklist */}
            <div style={{ border: "1.5px solid #003399", borderRadius: "3px", overflow: "hidden", marginBottom: "3mm" }}>
              <div style={{ background: "#003399", color: "white", padding: "1.5mm 4mm", fontSize: "8.5pt", fontWeight: "bold" }}>Documentation Checklist</div>
                ...
                <tbody>
                  {CHECKLIST_ROWS.map((row, i) => (
                    <tr key={i} style={{ borderBottom: i < CHECKLIST_ROWS.length - 1 ? "0.5px solid #ddd" : "none" }}>
                      ...
                      <td style={{ width: "50%", padding: "1.5mm 3mm" }}>
                        {row.right && (<><span style={{ display: "inline-block", width: "3mm", height: "3mm", border: "1px solid #333", marginRight: "2mm", verticalAlign: "middle" }} />{row.right}</>)}
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            {/* Register Entry */}
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "8pt", border: "1px solid #aaa" }}>
              <tbody>
                <tr>
                  <td style={{ padding: "2mm 3mm", borderRight: "1px solid #aaa", width: "50%" }}>
                    <strong>Loan Application Register</strong><br />
                    <span style={{ color: "#555" }}>Serial No.: </span>
                    <span style={{ borderBottom: "1px solid #333", display: "inline-block", minWidth: "28mm" }}>&nbsp;</span>
                  </td>
                  <td style={{ padding: "2mm 3mm", width: "50%" }}>
                    <strong>Document Execution Register</strong><br />
                    <span style={{ color: "#555" }}>Folio No.: </span>
                    <span style={{ borderBottom: "1px solid #333", display: "inline-block", minWidth: "14mm" }}>&nbsp;</span>
                    {" / "}
                    <span style={{ borderBottom: "1px solid #333", display: "inline-block", minWidth: "14mm" }}>&nbsp;</span>
                  </td>
                </tr>
            ...
            {/* Footer */}
            <div style={{ marginTop: "3mm", fontSize: "7pt", color: "#888", textAlign: "center", borderTop: "0.5px solid #ddd", paddingTop: "2mm" }}>
