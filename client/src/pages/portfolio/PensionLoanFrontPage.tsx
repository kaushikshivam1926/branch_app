/*
 * PensionLoanFrontPage.tsx
 * ─────────────────────────────────────────────────────────────────────────────
 * Renders a pre-filled A4 front-page cover sheet for a Pension Loan (PEN series)
 ...
 *
 * Pension-loan-specific fields:
 *   - PPO Number (editable)
 *   - Pension Account Number (auto-fetched from DEPOSIT_DATA by CIF)
 *   - Pension Paying Authority / Department (editable)
 *   - Type of Pension (editable: Superannuation / Family / Disability)
 *   - Monthly Pension Amount (editable)
 *   - Date of Retirement (editable)
 *   - Employer / Department (editable)
 *   - Pensioner's Age (editable)
 */
...
function buildPrintHTML(params: {
  ...
  mobile: string;
  ppoNumber: string;
  pensionAcctNo: string;
  pensionAcctDesc: string;
  ...
  const {
    ...
    sanctionAmt, instalAmt, intRate, sanctionDate, maturityDate,
    address, mobile, ppoNumber, pensionAcctNo, pensionAcctDesc,
    payingAuthority, pensionType, monthlyPension, retirementDate,
    employer, pensionerAge, logoB64,
  } = params;
  ...
  const detailRows: [string, string][] = [
    ...
    ["EMI Amount", instalAmt ? formatCurrency(instalAmt) : ""],
  ...
  const pensionRows: [string, string][] = [
    ["PPO Number", ppoNumber],
    ["Pension Account Number", pensionAcctNo],
    ["Pension Account Type", pensionAcctDesc],
    ["Pension Paying Authority", payingAuthority],
    ["Type of Pension", pensionType],
    ["Monthly Pension Amount (₹)", monthlyPension],
    ["Date of Retirement", retirementDate],
    ["Employer / Department", employer],
    ["Pensioner's Age (years)", pensionerAge],
  ];
  ...
  const pensionRowsHtml = pensionRows.map(([label, value]) => `
    <tr style="border-bottom:0.5px solid #e0e0e0;">
      <td style="padding:1.8mm 3mm;font-weight:600;color:#333;width:46mm;vertical-align:top;white-space:nowrap;border-right:0.5px solid #e0e0e0;background:#f0fff4;">${label}</td>
...
<div class="page">
  ...
  <div style="border-bottom:3px solid #1a7a4a;margin-bottom:4mm;padding-bottom:3mm;">
    <div style="display:flex;align-items:center;gap:8mm;">
      ...
      <div style="flex:1;">
        ...
        <div style="font-size:8.5pt;color:#555;margin-top:0.5mm;">Pension Loan File</div>
      </div>
      <div style="text-align:right;">
        <div style="font-size:17pt;font-weight:bold;color:#1a7a4a;border:2px solid #1a7a4a;padding:1.5mm 4mm;border-radius:4px;letter-spacing:1px;">${serialNo}</div>
        <div style="font-size:7pt;color:#888;margin-top:0.5mm;">Pension File No.</div>
      </div>
    </div>
  </div>
  ...
  <!-- Pension Details -->
  <div style="border:1.5px solid #1a7a4a;border-radius:3px;overflow:hidden;margin-bottom:3mm;">
    <div style="background:#1a7a4a;color:white;padding:1.5mm 4mm;font-size:8.5pt;font-weight:bold;letter-spacing:0.3px;">Pension Details</div>
    <table style="font-size:8pt;border:0.5px solid #e0e0e0;"><tbody>${pensionRowsHtml}</tbody></table>
  </div>
...
export default function PensionLoanFrontPage({ record, onClose }: Props) {
  const [loanData, setLoanData] = useState<LoanDataRecord | null>(null);
  const [loading, setLoading] = useState(true);
  const [printing, setPrinting] = useState(false);
  const iframeRef = useRef<HTMLIFrameElement>(null);

  // Pension-specific editable fields
  const [ppoNumber, setPpoNumber] = useState("");
  const [pensionAcctNo, setPensionAcctNo] = useState("");
  const [pensionAcctDesc, setPensionAcctDesc] = useState("");
  const [payingAuthority, setPayingAuthority] = useState("");
  const [pensionType, setPensionType] = useState("Superannuation");
  const [monthlyPension, setMonthlyPension] = useState("");
  const [retirementDate, setRetirementDate] = useState("");
  const [employer, setEmployer] = useState("");
  const [pensionerAge, setPensionerAge] = useState("");
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // Try to find pension account (Senior Citizen flag) in DEPOSIT_DATA
        const cif = match?.CIF || record.cifNo;
        if (cif) {
          const allDeposits: DepositRecord[] = await getAllRecords(STORES.DEPOSIT_DATA);
          const pensionAcct = allDeposits.find(
            d => d.CIF === cif && (d.SeniorCitizen_Flag === "SeniorCitizen" || (d.Acct_Desc || "").toUpperCase().includes("PENSION"))
          );
          if (pensionAcct) {
            setPensionAcctNo(pensionAcct.AcNo || "");
            setPensionAcctDesc(pensionAcct.Acct_Desc || "");
          }
        }
      } catch (err) {
  ...
  const instalAmt = loanData?.INSTALAMT || 0;
  ...
  const handlePrint = () => {
    ...
    const html = buildPrintHTML({
      serialNo: record.serialNo, custName, cifNo, acctNo, acctDesc,
  ...
  const inputStyle: React.CSSProperties = {
    width: "100%", border: "none", borderBottom: "1.5px solid #1a7a4a",
    outline: "none", fontSize: "8.5pt", fontFamily: "Arial, Helvetica, sans-serif",
    color: "#111", background: "transparent", padding: "0.5mm 0",
  };

  const PenRow = ({ label, value, onChange, placeholder, type = "text" }: {
    label: string; value: string; onChange: (v: string) => void; placeholder?: string; type?: string;
  }) => (
    <tr style={{ borderBottom: "0.5px solid #e0e0e0" }}>
      <td style={{ padding: "1.8mm 3mm", fontWeight: 600, color: "#333", width: "46mm", verticalAlign: "top", whiteSpace: "nowrap", borderRight: "0.5px solid #e0e0e0", background: "#f0fff4" }}>
        {label}
      </td>
      ...
      <div className="fixed inset-0 z-50 bg-black/70 flex flex-col items-center justify-start overflow-auto py-6 px-4">
        ...
        <div className="w-full max-w-[220mm] flex items-center justify-between mb-4 flex-shrink-0">
          <div className="text-white font-semibold text-lg">
            Pension Loan Front Page — <span className="font-mono text-green-300">{record.serialNo}</span>
          </div>
          <div className="flex gap-2">
            <button onClick={handlePrint} disabled={loading || printing}
              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-green-700 text-white text-sm font-semibold hover:bg-green-800 transition-colors disabled:opacity-50">
        ...
        {loading ? (
          ...
          <div className="bg-white shadow-2xl rounded flex-shrink-0"
            ...
            {/* Header */}
            <div style={{ borderBottom: "3px solid #1a7a4a", marginBottom: "4mm", paddingBottom: "3mm" }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8mm" }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: "8.5pt", color: "#555", marginTop: "0.5mm" }}>Pension Loan File</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: "17pt", fontWeight: "bold", color: "#1a7a4a", border: "2px solid #1a7a4a", padding: "1.5mm 4mm", borderRadius: "4px", letterSpacing: "1px" }}>
                    {record.serialNo}
                  </div>
                  <div style={{ fontSize: "7pt", color: "#888", marginTop: "0.5mm" }}>Pension File No.</div>
                </div>
              </div>
            </div>
            ...
            {/* Pension Details */}
            <div style={{ border: "1.5px solid #1a7a4a", borderRadius: "3px", overflow: "hidden", marginBottom: "3mm" }}>
              <div style={{ background: "#1a7a4a", color: "white", padding: "1.5mm 4mm", fontSize: "8.5pt", fontWeight: "bold" }}>Pension Details</div>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "8pt", border: "0.5px solid #e0e0e0" }}>
                <tbody>
                  <PenRow label="PPO Number" value={ppoNumber} onChange={setPpoNumber} />
                  <PenRow label="Pension Account Number" value={pensionAcctNo} onChange={setPensionAcctNo} />
                  <PenRow label="Pension Account Type" value={pensionAcctDesc} onChange={setPensionAcctDesc} />
                  <PenRow label="Pension Paying Authority" value={payingAuthority} onChange={setPayingAuthority} placeholder="e.g. CPPC, Treasury…" />
                  <PenRow label="Type of Pension" value={pensionType} onChange={setPensionType} type="select" />
                  <PenRow label="Monthly Pension Amount (₹)" value={monthlyPension} onChange={setMonthlyPension} placeholder="e.g. 25,000" />
                  <PenRow label="Date of Retirement" value={retirementDate} onChange={setRetirementDate} placeholder="DD/MM/YYYY" />
                  <PenRow label="Employer / Department" value={employer} onChange={setEmployer} placeholder="e.g. Govt. of Maharashtra" />
                  <PenRow label="Pensioner's Age (years)" value={pensionerAge} onChange={setPensionerAge} placeholder="e.g. 65" />
                </tbody>
              </table>
            </div>
