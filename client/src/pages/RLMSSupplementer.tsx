...
const PDF_COORDINATES: Record<string, { page: number; x: number; y: number; width: number; height: number }> = {
  ...
  disbursementDate: { page: 2, x: 435, y: 623, width: 45, height: 15 },
  ...
  repaymentTenure: { page: 4, x: 161, y: 629, width: 125, height: 20 },
...
const extractDataByCoordinates = (pagesItems: Record<number, { str: string; x: number; y: number }[]>) => {
  ...
  for (const [key, box] of Object.entries(PDF_COORDINATES)) {
    ...
    const matchedItems = pageItems.filter(item => item.x >= left && item.x <= right && item.y >= bottom && item.y <= top);
  ...
  const handlePdfUpload = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      for (let i = 1; i <= pdf.numPages; i++) {
        ...
        pagesItems[i - 1] = (textContent.items as { str: string; transform: number[] }[]).map(item => ({
    ...
    try {
      ...
      for (const [key, val] of Object.entries(data)) {
        if (!readOnlyFields[key as keyof DataModel] && PDF_COORDINATES[key] && val) {
          ...
          if (pages[page]) {
            pages[page].drawText(String(val), { x: x + 2, y: y - 10, size: 8, font, color: rgb(0, 0, 0), maxWidth: width, lineHeight: 12 });
          }
        }
      }
